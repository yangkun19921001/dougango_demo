		...
		try{
			...
			if(NgnApplication.getSDKVersion() >=5 && activity != null){
				/*synchronized(mContacts)*/{
					...
					managedCursor = activity.managedQuery(CommonDataKinds.Phone.CONTENT_URI,
					...
					int indexPhoneContactId = managedCursor.getColumnIndex(CommonDataKinds.Phone.CONTACT_ID);
					int indexPhoneType = managedCursor .getColumnIndex(CommonDataKinds.Phone.TYPE);
					int indexPhoneLabel = managedCursor.getColumnIndex(CommonDataKinds.Phone.LABEL);
					int indexPhoneNumber = managedCursor.getColumnIndex(CommonDataKinds.Phone.NUMBER);
					int indexPhonePhotoId = managedCursor .getColumnIndex(CommonDataKinds.Phone.PHOTO_ID);
					 ...
					 while(managedCursor.moveToNext()){
						 ...
						 if(phoneNumber != null){
							...
							if(NgnStringUtils.isNullOrEmpty(label)){
								final int resId = CommonDataKinds.Phone.getTypeLabelResource(type);
